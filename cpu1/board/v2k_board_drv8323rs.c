//=============================================================================
// v2k_board_drv8323rs.c - DRV8323RS GPIO and bounded TI driver adapter
//=============================================================================
...
#define V2K_BOARD_DRV_WAKE_CYCLES         (DEVICE_SYSCLK_FREQ / 1000uL)
#define V2K_BOARD_DRV_SLEEP_CYCLES        (DEVICE_SYSCLK_FREQ / 1000uL)
#define V2K_BOARD_DRV_TX_FIFO_DELAY       0x10u

#define V2K_BOARD_DRV_STATUS0_FAULT_MASK \
...
#define V2K_BOARD_DRV_STATUS1_FAULT_MASK \
...
static void v2k_board_drv8323rs_init_driver(void)
{
...
static void v2k_board_drv8323rs_prepare_spi(void)
{
    v2k_board_drv8323rs_init_driver();
    ...
    if (s_spi_configured == 0u)
    {
        ...
        SPI_setTxFifoTransmitDelay(DRV_SPI_BASE, V2K_BOARD_DRV_TX_FIFO_DELAY);
        SPI_clearInterruptStatus(DRV_SPI_BASE, SPI_INT_TXFF);
        s_spi_configured = 1u;
    }
}

static uint16_t v2k_board_drv8323rs_read(DRV8323_Address_e reg, uint16_t *value)
{
    DRV8323_Obj *obj;

    v2k_board_drv8323rs_prepare_spi();
...
static void v2k_board_drv8323rs_apply_official_baseline(void)
{
...
static uint16_t v2k_board_drv8323rs_write_baseline(void)
{
    DRV8323_Obj *obj;
...
uint16_t v2k_board_drv8323rs_poll(void)
{
    ...
    if ((v2k_board_drv8323rs_read(DRV8323_ADDRESS_STATUS_0, &status1) == 0u) ||
        (v2k_board_drv8323rs_read(DRV8323_ADDRESS_STATUS_1, &status2) == 0u))
    {
        return 0u;
    }
...
uint16_t v2k_board_drv8323rs_configure_and_verify(void)
{
    ...
    if ((v2k_board_drv8323rs_read(DRV8323_ADDRESS_CONTROL_2, &control) == 0u) ||
        (v2k_board_drv8323rs_read(DRV8323_ADDRESS_CONTROL_3, &gate_hs) == 0u) ||
        (v2k_board_drv8323rs_read(DRV8323_ADDRESS_CONTROL_4, &gate_ls) == 0u) ||
        (v2k_board_drv8323rs_read(DRV8323_ADDRESS_CONTROL_5, &ocp) == 0u) ||
        (v2k_board_drv8323rs_read(DRV8323_ADDRESS_CONTROL_6, &csa) == 0u))
    {
        return 0u;
    }
    ...
    v2k_board_drv8323rs_apply_official_baseline();
    ...
    if (v2k_board_drv8323rs_write_baseline() == 0u)
    {
        return 0u;
    }
...
{
    return (((s_status1 & V2K_BOARD_DRV_STATUS0_FAULT_MASK) != 0u) ||
            ((s_status2 & V2K_BOARD_DRV_STATUS1_FAULT_MASK) != 0u)) ? 1u : 0u;
}
