//=============================================================================
// wire_adc.c - F28P65x motor ADC schedule validation and ISR ownership
//=============================================================================
...
static uint16_t wire_adc_count_in_current_window(uint16_t count)
{
    return (count >= WIRE_CURRENT_LIMIT_LOW_COUNTS) &&
           (count <= WIRE_CURRENT_LIMIT_HIGH_COUNTS);
}

static uint16_t wire_adc_soc_trigger_matches(uint32_t base,
                                             uint16_t soc,
                                             uint16_t trigger)
{
...
static uint16_t wire_adc_control_interrupt_is_valid(void)
{
...
{
    return
        wire_adc_soc_trigger_matches(myADC0_BASE, myADC0_SOC0,
                                     ADC_TRIGGER_EPWM1_SOCA) &&
        wire_adc_soc_trigger_matches(myADC0_BASE, myADC0_SOC1,
                                     ADC_TRIGGER_EPWM1_SOCA) &&
        wire_adc_soc_trigger_matches(myADC0_BASE, myADC0_SOC2,
                                     ADC_TRIGGER_EPWM1_SOCA) &&
        wire_adc_soc_trigger_matches(myADC0_BASE, myADC0_SOC3,
                                     ADC_TRIGGER_EPWM1_SOCA) &&
        wire_adc_soc_trigger_matches(myADC1_BASE, myADC1_SOC0,
                                     ADC_TRIGGER_EPWM1_SOCA) &&
        wire_adc_soc_trigger_matches(myADC1_BASE, myADC1_SOC1,
                                     ADC_TRIGGER_EPWM1_SOCA) &&
        wire_adc_soc_trigger_matches(myADC2_BASE, myADC2_SOC0,
                                     ADC_TRIGGER_EPWM1_SOCA) &&
        wire_adc_control_interrupt_is_valid();
}
...
uint16_t wire_adc_current_limit_config_is_valid(void)
{
    ...
    return
        (high == WIRE_CURRENT_LIMIT_HIGH_COUNTS) &&
        (low == WIRE_CURRENT_LIMIT_LOW_COUNTS) &&
...
uint16_t wire_adc_current_window_is_valid(void)
{
    ...
    return wire_adc_count_in_current_window(phase_a) &&
           wire_adc_count_in_current_window(phase_b) &&
           wire_adc_count_in_current_window(phase_c);
}
...
uint16_t wire_adc_current_limit_status(void)
{
    ...
    {
        sources |= WIRE_CURRENT_SOURCE_PHASE_C_HIGH;
    }
    if ((status & ADC_EVT_TRIPLO) != 0u)
    {
        sources |= WIRE_CURRENT_SOURCE_PHASE_C_LOW;
    }
    return sources;
}
