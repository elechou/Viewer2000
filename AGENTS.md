...
- **Phase 5.0 — Power-stage hardware interface** ([plan](docs/phase5.0-powerstage-interface.md), accepted 2026-06-22): the three-phase PWM/ADC/protection substrate, DRV8323RS, and non-blocking AS5600 driver passed every verification executable with the available bench equipment. The production interface contains no bench-only diagnostic commands. The checked-in default remains DRY_RUN; calibrated current limits and differential-probe/oscilloscope measurements remain gates before sustained powered motor operation. No sustained V/f or FOC runs belong to this phase. The cross-cutting power-stage protection model (trip-zone funnel, the three hardware trip chains, DCAEVT1's dual identity, fault discrimination, and the POWERED gates) is consolidated in [protection-architecture.md](docs/protection-architecture.md); read it before touching any trip-zone/CMPSS/X-BAR/DRV code or moving from DRY_RUN to POWERED.
- **Phase 5 — Motor bring-up (platform acceptance)**: open-loop V/f → current-sense calibration → current loop → encoder → speed loop → position loop. Each step is a small user application, validating the platform interface design along the way.
