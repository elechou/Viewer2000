...
- the current Phase 3.5 temporary code calls GPIO/CPUSEL driverlib directly in `cpu1/runtime/v2k_main.c`. This violates the project principle "SysConfig owns static hardware".
...
The pad config must also be generated by SysConfig. If the GUI uses a default pull-up on the SCI pins, go by the generated result and verify it on hardware; any intentional adjustment goes back to SysConfig to change, not an appended GPIO override in `v2k_main.c` or `cpu2.c`.
...
After the SysConfig migration, `cpu1/runtime/v2k_main.c` must no longer contain:
