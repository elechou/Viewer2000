...
前期参数整定、状态机指令、变量观测通过 CCS 实时模式（CPU 不停机读写内存）+ Expressions / Graph 窗口完成。但与前身项目不同：**线上协议与四个共享内存接口同在 Phase 0 定稿**（上位机需求已知——myway_viewer 在手），最小 SCI 数据泵提前到 Phase 3.5，不写一次性 ASCII 解析器。
